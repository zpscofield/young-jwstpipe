...
run_pipeline() {
    ...
    if ! should_skip_step "fnoise_correction"; then
        ...
        echo "  ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯  "
        python "$BASE_DIR/utils/remstriping_update_parallel.py" --runall --nproc "$FNOISE_NPROC" --output_dir "$OBS_DIR/stage1_output"
        echo ""
    else
    ...
    if ! should_skip_step "wisp_subtraction"; then
        ...
        echo "  ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯  "
        python "$BASE_DIR/utils/subtract_wisp.py" --files $OBS_DIR/stage2_output/jw*cal.fits --wisp_dir "$BASE_DIR/utils/wisp-templates" --output_dir "$OBS_DIR/stage2_output" --suffix "_final" --nproc "$WISP_NPROC"
        echo ""
    else
    ...
    if ! should_skip_step "background_subtraction"; then
        ...
        echo "  ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯  "
        python "$BASE_DIR/utils/bkg_sub_parallel.py" --input_dir "$OBS_DIR/stage2_output" --nproc "$BKG_NPROC" --output_dir "$OBS_DIR/stage2_output"
        echo ""
    else
    ...
    if ! should_skip_step "download_cal_references"; then
        ...
        echo "  ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯  "
        crds bestrefs --files $OBS_DIR/stage2_output/jw*cal_final.fits --sync-references=1
        echo ""
    else
