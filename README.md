...
**Test before a full run.** **Save & Test pipeline** runs every enabled step on a small subset of the data: two exposures per filter, each contributing one short-wavelength detector (a wisp-affected one where available) and its long-wavelength partner. Outputs go to `<output>/<observation>_test`, so nothing from a real reduction is touched. It takes minutes rather than hours and catches configuration, path, CRDS, and environment problems early. When it succeeds, a **Run full pipeline now** button starts the full reduction with the same settings. The same thing from the command line is `./young_pipeline.sh --test`.
