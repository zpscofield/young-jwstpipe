...
def make_segmap(f, log, seg_from_lw=True, sigma=0.8, npixels=10, dilate_segmap=5, save_segmap=False):
    ...
    log.info('Making segmap for {}'.format(f))
    ...
    if (seg_from_lw) & ('long' not in detector):
        try:
            if 'a' in detector:
                f_lw = f.replace(detector.lower(), 'nrcalong').replace('_cal.fits', '_cal_wisp.fits')
            if 'b' in detector:
                f_lw = f.replace(detector.lower(), 'nrcblong').replace('_cal.fits', '_cal_wisp.fits')
...
def process_file(log, f, output_dir, wisp_dir='./', create_segmap=True, seg_from_lw=True, sigma=0.8, npixels=10, dilate_segmap=5,
    ...
    log.info('Processing complete for {}'.format(f))
...
def process_files(log, files, nproc, **kwargs):
    ...
    relevant_files = [f for f in files if any(substring in f for substring in ['nrca3', 'nrca4', 'nrcb3', 'nrcb4'])]
    ...
    for file in non_relevant_files:
        if file.endswith('_cal.fits'):
            new_filename = file.replace('_cal.fits', '_cal_wisp.fits')
...
def subtract_wisp(log, f, wisp_data, segmap_data=None, sub_wisp=True, gauss_smooth_wisp=False, gauss_stddev=3.0, scale_wisp=True,
    ...
    # Get the input data
    log.info('Applying wisp template to {}'.format(f))
    h = fits.open(f)
    data = h['SCI'].data
    dq = h['DQ'].data
    if segmap_data is None:
        log.info('Warning: No segmap data given for {}. Assuming no sources.'.format(f))
